...
// MongoDB Connection
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/poultry-farm-ledger';
...
app.post('/api/records', async (req, res) => {
  try {
    const { date, truckId, invoiceNo, weights, buckleNumber, buckleWeight } = req.body;
    
    // Validate required fields
    if (!date || !truckId || !invoiceNo || !weights || !Array.isArray(weights)) {
      return res.status(400).json({ error: 'Missing required fields' });
    }

    // Calculate derived fields
    const calculatedFields = calculateFields({ weights, buckleNumber, buckleWeight });
